...
resp_cars = requests.get(f"{API}/cars")
...
with st.form("damage_form"):
    ...
    if submitted:
        if not selected_damages:
            ...
            # Send POST request
            resp = requests.post(f"{API}/car-damages", json=payload)
...
resp_totals = requests.get(f"{API}/car-damages")
if resp_totals.status_code == 200:
